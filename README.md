...
| :--- | :---: | :--- |
| [**SQL Injection**](./Labs/web-server-attack/sql-injection/README.md) | ✅ Done | Default ruleset (`31106`) gak detect fase recon (`'`, `ORDER BY`); severity gak proporsional antara recon vs data exfiltration; custom Wazuh rule lagi dibangun bertahap |
| [**SQLi → `INTO OUTFILE` Webshell Attempt**](./Labs/web-server-attack/sqli-into-outfile-webshell/README.md) | ✅ Done | Eskalasi SQLi ke RCE lewat `INTO OUTFILE` **gagal** — privilege `FILE` gak nempel ke akun `dvwa`@`localhost` (grant di-scope ke `dvwa.*`, bukan global). Defense-in-depth gak sengaja: SQLi tetep berhasil (baca data), tapi privilege scoping database ngebatesin blast radius sampe gak bisa tulis file. Tetep ke-detect (`31106`, level 6) walau exploit-nya gagal |
| [**Command Injection**](./Labs/web-server-attack/command-injection/README.md) | ✅ Done | Command tereksekusi penuh (RCE, sampai arbitrary file write) + **reverse shell** (LOLBin: `sh`+`nc`+`mkfifo`); request **POST** gak ninggalin jejak payload di `access.log`, dan proses/koneksi reverse shell gak ke-detect sama sekali walau keliatan di `htop` — blind spot total, jadi basis penambahan **NIDS + auditd**. **Remediasi confirmed**: replay skenario yang sama sekarang ke-detect via auditd (rule `100300`) + Suricata custom rule (`100400`/SID `1000003`) |
| [**File Inclusion (LFI)**](./Labs/web-server-attack/file-inclusion/README.md) | ✅ Done | Path traversal ke `/etc/passwd` berhasil & ke-detect (`31106`); log poisoning gagal (`Permission denied`, tapi tetap ke-log); RCE akhirnya tercapai lewat kombinasi LFI + Command Injection (webshell). RFI di-skip (`allow_url_include` off default). **Generalisasi rule Command Injection**: auditd (`100300`) confirmed generalisasi (deteksi di layer proses OS), Suricata (`1000003`) **tidak** generalisasi (scope-nya spesifik POST body, LFI lewat GET gak match — butuh rule tambahan) |
| **XSS** ([Reflected](./Labs/web-server-attack/xss/reflected/README.md) → [Stored](./Labs/web-server-attack/xss/stored/README.md) → [DOM](./Labs/web-server-attack/xss/dom/README.md)) | ✅ Done | Reflected: rule `31105`/`31106` ke-detect langsung. Stored: blind spot **replay** ketemu (server gak pernah tau ada payload aktif pas visitor lain buka halaman) — ditutup pake **CSP Report-Only** (browser ngirim violation report independen dari request awal). DOM: fragment (`#...`) gak otomatis exploitable, cuma jalan lewat kombinasi reload+query string, dan request ke server tetap 100% bersih. **Gap detection engineering**: rule CSP `100404` awalnya asumsi "`script-src-elem` = selalu high-confidence", ternyata false positive 100% di modul yang punya inline `<script>` legit sendiri — di-tuning jadi kombinasi blocklist (`100407`) + allowlist (`100408`) |
| [**JavaScript Attacks**](./Labs/web-server-attack/javascript-attacks/README.md) | ✅ Done | Client-side trust bypass (`md5(rot13(phrase))`) confirmed — token dihitung ulang di Python tanpa buka browser sama sekali, server tetap nerima. Deteksi behavioral (`User-Agent`/`Referer`, bukan payload) confirmed jalan 3 tier (`100501`-`100503`). **Blind spot gak terduga**: decoder bawaan Wazuh gak nyimpen `Referer`/`User-Agent` (butuh sibling decoder tambahan) dan rule custom gak bisa menang ngelawan rule bawaan (`31108`) kalau didesain independen (butuh numpang `if_sid`) — insight: nulis rule yang "benar secara logika" gak cukup tanpa ngerti mesin resolusi SIEM-nya sendiri |
| [**File Upload**](./Labs/web-server-attack/file-upload/README.md) | ✅ Done | Webshell **b374k** berhasil diupload (bypass hidden field `MAX_FILE_SIZE` via `curl`) dan RCE penuh via UI terminal-nya. **FIM/syscheck** (layer baru, filesystem) confirmed jadi sinyal independen (`554`, "File added") terlepas dari isi request maupun proses; **auditd** (`100300`) confirmed generalisasi penuh ke vektor baru; **Suricata** confirmed kosong total (command dieksekusi dari dalam shell, gak lewat request HTTP baru). `access.log` sempet gagal divalidasi pas Wazuh Manager down, tapi retest berhasil: anomali `User-Agent`/`Referer` di request upload (rapuh, gampang di-spoof) dan anomali method+path (`POST` ke direktori upload, lebih robust) sama-sama confirmed jadi sinyal tambahan |

Klik nama lab di kolom pertama buat baca writeup lengkapnya (payload, step-by-step, GIF evidence).
...
| FIM/syscheck — File Integrity Monitoring di Web-Server | ✅ Done | Nangkep file baru di direktori upload (`realtime` + `whodata`), confirmed jadi sinyal independen di lab File Upload (rule `554`) — lihat [`web-server-fim-setup.md`](./Infrastructure/web-server-fim-setup.md) |
